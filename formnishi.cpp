...
pdb_nishi::pdb_nishi(const char *pdbname, vector<int> num_pol){
    ...
    // formnishi
    int count_resnum=1,count_pol=0,buf_rnum[2];
    buf_rnum[0]=1;
    char count_chainid='A';
    //
	...
	while( fgets(buf1,sizeof(buf1),fin)!=NULL ){
        ...
        // formnishi
        if(count_resnum>num_pol[count_pol] && count_pol < (int)num_pol.size()){
          count_pol ++;
          count_resnum = 1;
          count_chainid ++;
        }
